var Spider ={
	...
	initSplash:function()
	{
		...
		TweenLite.to(splash,0.1,{css:{alpha:0},onComplete:Spider.splashReady});
	...
	splashReady:function()
	{
		...
		TweenLite.to(splash,0.5,{css:{alpha:1},onComplete:Spider.splashShown});
	},
	splashShown:function()
	{
		...
		TweenLite.to(splash,0.5,{delay:Spider.data.splash.duration,css:{alpha:0},onComplete:Spider.splashComplete});
	},
	...
	toast:function(str)
	{
		...
		var toastHandler = function()
		{
			...
			TweenLite.to(toast,0.5,{delay:Spider.data.toastDuration,css:{alpha:0},onComplete:destroy});
		}
		TweenLite.to(toast,0.5,{css:{alpha:1},onComplete:toastHandler});
	},
	...
	navigateTo:function(index)
	{
		var content =document.getElementById(Spider.data.id.content);
			...
			TweenLite.to(content,0.5,{css:{left:x + "px"},onComplete:Spider.controller.handler.onPageChange});
	},
	resize:function()
	{
...
Spider.event=
{
	...
	{
		if(!Spider.event.callback[eventName])Spider.event.callback[eventName]=[];
...
Spider.element=
{
	...
	allDescendants:function (node,id,name) {
    	for (var i = 0; i < node.childNodes.length; i++) {
     	 ...
     	 if(child.setAttribute && !child.getAttribute(name!=null?name:Spider.data.att.swipeParent,id))child.setAttribute(name!=null?name:Spider.data.att.swipeParent,id);
	...
	getAllElementsWithAttribute:function(attribute,name)
	{
	  ...
	  for (var i = 0; i < allElements.length; i++)
	  {
	    ...
	    {
	    	if(name!=null && allElements[i].getAttribute(attribute) ==name)return allElements[i];
...
Spider.style=
{
	...
	setScroller:function(scroller)
	{
		...
		if(!Spider.data.className.scroller)scroller.style.backgroundColor="#333";
	},
...
Spider.controller=
{
	...
	init:function()
	{
		...
		var content =document.getElementById(Spider.data.id.content);
		if(content.getAttribute('swipe') && content.getAttribute('swipe')=="true")this.swipeMode=true;
	},
	...
	gestureSwipe:function(item,event)
	{
		if(Spider.controller.isDrag){
			...
			if(Spider.event.callback[Spider.event.type.mouseup] && Spider.event.callback[Spider.event.type.mouseup]['document'])Spider.event.callback[Spider.event.type.mouseup]['document'](null,event);
			return;
		}
		...
		if(module && module.id!=Spider.data.id.content && module && module.getAttribute && module.getAttribute(Spider.data.att.swipeParent))
		{
			...
			TweenLite.to(parent,0.2,{css:{left:x + "px"}});
		...
		}else if(this.swipeMode)
		{
			...
			if (y > 0 )y=0;
			if(y < -(page.clientHeight - Utensil.stageHeight())) y=-(page.clientHeight - Utensil.stageHeight());
			TweenLite.to(content,0.1,{css:{top:y + "px"},onComplete:Spider.controller.handler.onPageChange});
	...
	gestureScroll:function(item)
	{
		...
		if(Spider.controller.isDrag)
		{
			...
			var x = (left-item.x);
			if(x<0)x=0;
			if(x>element.parentNode.clientWidth)x=element.parentNode.clientWidth-element.clientWidth;
			element.style.left = x+"px";
			if(filler)filler.style.width = (x+(element.clientWidth * 0.5))+"px";
			...
			if(Spider.event.callback[Spider.event.type.drag] && Spider.event.callback[Spider.event.type.drag][element.id])Spider.event.callback[Spider.event.type.drag][element.id]({percent:percent},event);		
			...
			if(page && page.getAttribute && page.getAttribute(Spider.data.att.scrollParent))
			{
				...
				//}
					if (y > 0 )y=0;
					if(y < -(page.clientHeight - page.parentNode.clientHeight)) y=-(page.clientHeight - page.parentNode.clientHeight);
					scroller.style.top = (-((page.parentNode.clientHeight/page.clientHeight) * Number(y))) + "px";
					TweenLite.to(page,0.1,{css:{top:y + "px"}});
			}
