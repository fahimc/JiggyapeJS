var Spider ={
	...
	toast:function(str)
	{
		...
		var toastHandler = function()
		{
			...
			TweenLite.to(toast,1,{delay:3,css:{alpha:0},onComplete:destroy});
		}
		TweenLite.to(toast,1,{css:{alpha:1},onComplete:toastHandler});
	},
...
Spider.element=
{
	...
	setScrollers:function()
	{
		...
		for(var a=0;a<nodes.length;a++)
		{
			...
			Spider.element.allDescendants(holder,Spider.data.scroller.index,Spider.data.att.scrollParent);
			if(content.getAttribute(Spider.data.att.scrollerIndex)==null)Spider.data.scroller.index++;
		}
...
Spider.controller=
{
	...
	gestureSwipe:function(item)
	{
		...
		if(module && module.id!=Spider.data.id.content && module && module.getAttribute && module.getAttribute(Spider.data.att.swipeParent))
		{
			...
			TweenLite.to(parent,0.5,{css:{left:x + "px"}});
		...
		}else if(this.swipeMode)
		{
			...
			TweenLite.to(content,0.1,{css:{top:y + "px"}});
	...
	gestureScroll:function(item)
	{
		...
		if(Spider.controller.isDrag)
		{
			...
			var percent = (x/(element.parentNode.clientWidth-element.clientWidth))*100;
	...
	gestureClick:function(item,event)
	{
		...
		Spider.event.callback[Spider.event.type.click][element.id]();
	},
	handler:
	{
