var gesture = {
	...
	init : function() {
		...
		this.createHandler("onScrollWheel");
		if ('ontouchstart' in document.documentElement) {
			// this.minSwipe = 5;
			document.addEventListener('touchstart', this.handlers.onMouseDown,
					true);
			document.addEventListener('touchend', this.handlers.onMouseUp,
					false);
		} else {
			Utensil.addListener(document.body, "mousedown",
					this.handlers.onMouseDown);
			Utensil.addListener(document, "mouseup", this.handlers.onMouseUp);
		}
		
		Utensil.addListener(document,this.mousewheelevt, this.handlers.onScrollWheel);
	},
	deactivate : function() {
		this.active = false;
	},
	...
	onMouseDown : function(event) {
		...
		if (this.target && this.target.className) {
		...
		if ('ontouchstart' in document.documentElement) {
			...
			document.addEventListener('touchmove', this.handlers.onMouseMove,
					false);
		} else {
			Utensil.addListener(document.body, "mousemove",
	...
	onMouseMove : function(event) {
		...
		if (Math.abs(xMovement) > Math.abs(yMovement)
				&& Math.abs(xMovement) >= this.minSwipe) {
			if (!this.dir || this.dir == this.TYPE_HORIZONTAL) {
				...
				&& Math.abs(yMovement) >= this.minSwipeUp) {
	...
	onMouseUp : function(event) {
		...
		if ('ontouchstart' in document.documentElement) {
			document
					.removeEventListener('touchmove', this.handlers.onMouseMove);
		} else {
			Utensil.removeListener(document.body, "mousemove",
					this.handlers.onMouseMove);
		}
	...
	callFunc : function(eventName, event) {
		for ( var a = 0; a < this.callbacks.length; a++) {
			if (this.callbacks[a].e == eventName) {
				this.callbacks[a].c[this.callbacks[a].m](this.currentItem,
						event);
			}
		}
	},
	createHandler : function(eventName) {
		...
		this.handlers[eventName] = function(event) {
			...
			if (target.id && gesture.ignoreList[target.id] || target.tagName
