var gesture = {
	...
	onMouseDown : function(event) {
		...
		if(event.touches && event.touches[0].target)this.target=event.touches[0].target;
	...
	onMouseUp : function(event) {
		this.target = event.srcElement ||event.target;
		if(this.target.tagName && this.target.tagName=="INPUT")return;
	...
	createHandler : function(eventName) {
		...
		this.handlers[eventName] = function(event) {
			...
			if(target.id && gesture.ignoreList[target.id]|| target.tagName && target.tagName=="INPUT")
				{
