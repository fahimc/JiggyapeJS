...
Jiggyape.view = {
	...
	createSearchItem : function(entry, index) {
		...
		addButton.setAttribute(Jiggyape.view.att.videoDuration,
	...
	addToPlaylist : function(title, videoURL, duration) {
		...
		Spider.event.addListener(shareButton.id, "click",
				Jiggyape.event.onPlaylistShareClicked);
...
Jiggyape.event = {
	...
	onPlaylistRemoveClicked : function(item, event) {
		...
		if (YoutubePlayerJS.player.getPlaylistIndex() >= Number(index) + 1) {
			Jiggyape.view.PlayListView.removeSelectedButton();
			YoutubePlayerJS.player
					.playVideoAt(Jiggyape.data.player.playlist.length - 1);
			Jiggyape.data.currentIndex = Jiggyape.data.player.playlist.length - 1;
			var n = document.getElementById("playButton-"
					+ (Jiggyape.data.currentIndex - 1));
			n.className += " smallControlSelected";
		}
		YoutubePlayerJS.player.loadPlaylist(Jiggyape.data.player.playlist,
				Jiggyape.data.currentIndex);
		...
		for ( var a = 0; a < playList.childNodes.length; a++) {
			if (playList.childNodes[a].tagName == "LI") {
				var childIndex = playList.childNodes[a].id.replace(
						Jiggyape.view.id.playlistEntry, "");
				...
				if (childIndex && Number(childIndex) > Number(index)) {
					...
					playList.childNodes[a].id = Jiggyape.view.id.playlistEntry
							+ childIndex;
				}
	...
	onDragUp : function(item, event) {
		var seekerHandle = document
				.getElementById(Jiggyape.view.VideoView.seekerHandle);
...
Jiggyape.view.PlayListView = {
	removeSelectedButton : function() {
		var element = document.getElementById("playButton-"
				+ Number(YoutubePlayerJS.player.getPlaylistIndex() - 1));
		if (element)
	...
	nextSong : function() {
		...
		if (Jiggyape.data.currentIndex >= Jiggyape.data.player.playlist.length
				|| Jiggyape.data.player.playlist.length - 1 <= YoutubePlayerJS.player
						.getPlaylistIndex()) {
			Jiggyape.data.currentIndex = 1;
	...
	previousSong : function() {
		...
		if (YoutubePlayerJS.player.getPlaylistIndex() == 1) {
			...
			Jiggyape.data.currentIndex = YoutubePlayerJS.player
					.getPlaylistIndex() - 1;
...
Jiggyape.view.VideoView = {
	...
	onSeekUpdate : function(percent) {
		if (isNaN(percent) || Spider.controller.isDrag
				|| Jiggyape.view.VideoView.seeking)
			return null;

		if (percent > 1)
			percent = 1;
		var controlsHolder = document
				.getElementById(Jiggyape.view.VideoView.controlsHolderId);
		var seekerHandle = document
				.getElementById(Jiggyape.view.VideoView.seekerHandle);
		var filler = Spider.element.getDragFiller(seekerHandle.parentNode);
		seekerHandle.style.left = (seekerHandle.parentNode.clientWidth * percent)
				+ "px";

		if (filler)
			filler.style.width = ((seekerHandle.parentNode.clientWidth * percent) + (seekerHandle.clientWidth * 0.5))
					+ "px";

	},
	paused : function() {
		var playPauseButton = document
				.getElementById(Jiggyape.view.id.playPauseButtonId);
	...
	resize : function() {
		...
		player.style.height = (Utensil.stageHeight() - header.clientHeight
				- footer.clientHeight
				- Jiggyape.view.element.searchBar.clientHeight
				- controlsHolder.clientHeight - 10)
				+ "px";
	}
}
